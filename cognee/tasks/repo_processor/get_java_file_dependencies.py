...
from cognee.shared.JavaCodeGraphEntities import JavaFile, Repository


async def get_java_source_code_files(repo_path):
    """
    Retrieve Java source code files from the specified repository path.

    This function scans the given repository path for files that have the .java extension
    ...
    -----------
        ...
        - repo_path: The file path to the repository to search for Java source files.

    Returns:
    --------

        A list of absolute paths to .java files that contain source code, excluding empty
    ...
    java_files_paths = (
        ...
        if (
            ...
            and not file.endswith("Test.java")
            and ".venv" not in file
        )
    )

    source_code_files = set()
    for file_path in java_files_paths:
...
async def get_java_repo_file_dependencies(
    repo_path: str, detailed_extraction: bool = False
) -> AsyncGenerator[DataPoint, None]:
    """
    Generate a dependency graph for Java files in the given repository path.

    Check the validity of the repository path and yield a repository object followed by the
    dependencies of Java files within that repository. Raise a FileNotFoundError if the
    ...
    -----------
        ...
        - repo_path (str): The file path to the repository where Java files are located.
        - detailed_extraction (bool): A flag indicating whether to perform a detailed
          extraction of dependencies (default is False).
    """

    if not os.path.exists(repo_path):
        raise FileNotFoundError(f"Repository path {repo_path} does not exist.")

    source_code_files = await get_java_source_code_files(repo_path)
    ...
    from cognee.tasks.repo_processor.get_java_local_dependencies import get_java_local_script_dependencies

    for start_range, end_range in chunk_ranges:
        tasks = [
            get_java_local_script_dependencies(repo_path, file_path, detailed_extraction)
            for file_path in source_code_files[start_range : end_range + 1]
        ]

        results: list[JavaFile] = await asyncio.gather(*tasks)
