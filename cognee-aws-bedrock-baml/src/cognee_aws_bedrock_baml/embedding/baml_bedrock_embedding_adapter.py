"""AWS Bedrock Embedding Adapter for Cognee using BAML/LiteLLM"""
...
logger = get_logger("BamlBedrockEmbeddingAdapter")


class BamlBedrockEmbeddingAdapter(EmbeddingEngine):
    """
    AWS Bedrock Embedding Adapter for Cognee using BAML-compatible approach.

    Note: BAML focuses on LLM interactions, not embeddings directly.
    This adapter uses litellm for embeddings while maintaining compatibility
    with the BAML-based cognee-aws-bedrock-baml package.
    ...
    def __init__(
        ...
        """
        Initialize the BamlBedrockEmbeddingAdapter.
        ...
        logger.info(
            f"Initialized BamlBedrockEmbeddingAdapter with model={self.model}, "
            f"region={self.aws_region_name}, dimensions={self.dimensions}"
        )
